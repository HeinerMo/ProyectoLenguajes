	...
	BEGIN
		...
		IF  @param_campusId IS NULL
			BEGIN
				...
				WHERE e.TIME_END_EVENT >= GETDATE() --eventos que aún no han finalizado
			END
		ELSE
			BEGIN
							WITH usedSpots 
								...
								ON a.ID_ARTIST = ea.ID_ARTIST
									INNER JOIN usedSpots AS us
										ON us.eventId = e.ID_EVENT
											INNER JOIN tb_Recintos AS r
												ON r.ID_RECINTO = e.ID_RECINTO
				WHERE e.TIME_END_EVENT >= GETDATE() AND e.ID_RECINTO = @param_campusId --eventos que aún no han finalizado
			END
